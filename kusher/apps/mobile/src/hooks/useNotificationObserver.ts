...
//   const hashHydrated = useAuthStore((state) => state._hashHydrated);
...
export function useNotificationFeed() {
  ...
  const token = useAuthStore((state) => state.token);
  const hasHydrated = useAuthStore((state) => state._hasHydrated); // ✅ fix typo: _hashHydrated → _hasHydrated
  ...
  useEffect(() => {
    if (!hasHydrated || !token) return; // ✅ fix logic: was skipping when ready, now skips when NOT ready
